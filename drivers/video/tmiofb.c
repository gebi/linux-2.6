...
tmiofb_acc_wait(struct tmiofb_par *par, unsigned int ccs)
{
	if (in_atomic()) {
		int i = 0;
		while (ioread16(&par->lcr->ccs) > ccs) {
			udelay(1);
			i++;
			if (i > 10000) {
 ...
 */
static int
tmiofb_acc_write(struct tmiofb_par *par, const u32 *cmd, unsigned int count)
{
	struct tmio_lcr __iomem		*lcr	= par->lcr;
	int ret;

	ret = tmiofb_acc_wait(par, TMIOFB_FIFO_SIZE - count);
	if (ret)
		return ret;
...
tmiofb_fillrect(struct fb_info *fbi, const struct fb_fillrect *rect)
{
	...
	tmiofb_acc_write(fbi->par, cmd, ARRAY_SIZE(cmd));
}
		...
		unsigned int cmd, unsigned long arg)
{
	...
	case FBIO_TMIO_ACC_WRITE: {
		...
		return tmiofb_acc_write(fbi->par, acc, len);
	}
#endif
	}
...
static int tmiofb_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	acquire_console_sem();
	par->save = vmalloc(info->screen_size);
	if (!par->save) {
		release_console_sem();
		return -ENOMEM;
	}
	...
	ioread16_rep(par->vram, par->save, info->screen_size/2);
...
static int tmiofb_resume(struct platform_device *dev)
{
	...
	iowrite16_rep(par->vram, par->save, info->screen_size/2);
	vfree(par->save);
